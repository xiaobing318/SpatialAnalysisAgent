...
def raster_calc():
    ...
    dem_path = "D:/Data/QGIS-Sample-Data-master/QGIS-Sample-Data-master/qgis_sample_data/raster/SR_50M_alaska_nad.tif"  # DEM is used as the input
    ...
    slope_result = processing.run("gdal:slope", slope_params)
    # Load the output slope layer
    ...
    aspect_result = processing.run("gdal:aspect", aspect_params)
    # Load the output aspect layer
    aspect_layer = QgsRasterLayer(aspect_result['OUTPUT'], 'Aspect Layer')

    raster_calc_params = {
        'LAYERS': [slope_layer, aspect_layer],
        'EXPRESSION': '("Aspect Layer@1">200) AND ("Slope Output@1"<0.3)',
        'EXTENT': None,
        'CELL_SIZE': None,
        'CRS': None,
        'OUTPUT': 'TEMPORARY_OUTPUT'
    }
    raster_calc_result = processing.run("native:rastercalc", raster_calc_params)
    raster_calc_layer = QgsRasterLayer(raster_calc_result['OUTPUT'], 'Raster Calculator Result')
    # Add the raster calculator result to the QGIS interface
...
def raster_calc():
    ...
    # Calculate Slope
    slope_output_path = 'D:/SpatialAnalysisAgent/Output/slope.tif'
    slope_params = {
        'INPUT': dem_layer,
        'Z_FACTOR': 1.0,
    ...
    slope_layer = QgsRasterLayer(slope_output_path, 'Slope Layer')

    # Calculate Aspect
    aspect_output_path = 'D:/SpatialAnalysisAgent/Output/aspect.tif'
    aspect_params = {
        'INPUT': dem_layer,
        'OUTPUT': aspect_output_path
    }
    processing.run("gdal:aspect", aspect_params)
    aspect_layer = QgsRasterLayer(aspect_output_path, 'Aspect Layer')

    #Perform raster calculator
    raster_calc_params = {
    'LAYERS':[slope_layer, aspect_layer],
    'EXPRESSION':'("aspect@1">200) AND ("slope@1"<0.3)', #using the base name of the files are used to reffered to the raster files 
    'EXTENT':None,
    'CELL_SIZE':None,
    'CRS':None,
    'OUTPUT':'TEMPORARY_OUTPUT'
    }
    raster_calc_result = processing.run("qgis:rastercalculator", raster_calc_params)
    raster_calc_layer = QgsRasterLayer(raster_calc_result['OUTPUT'], 'Raster Calculator Result')
    # Add the raster calculator result to the QGIS interface
    QgsProject.instance().addMapLayer(raster_calc_layer)
raster_calc()
...
def raster_calc():
    ...
    # Calculate Slope
    slope_output_path = os.path.join(os.getcwd(),'slope.tif')
    slope_params = {
        'INPUT': dem_layer,
        'Z_FACTOR': 1.0,
        'OUTPUT': slope_output_path
    ...
    # Calculate Aspect
    aspect_output_path = os.path.join(os.getcwd(),'aspect.tif')
    aspect_params = {
        'INPUT': dem_layer,
        'OUTPUT': aspect_output_path
    }
    ...
    #raster calculator
    raster_calc_params = {
    'INPUT_A':aspect_output_path,
    'BAND_A':1,
    'INPUT_B': slope_output_path,
    'BAND_B':1,
    'FORMULA':'(A>100) & (B<0.3)',
    'OUTPUT':'TEMPORARY_OUTPUT'
    }
    raster_calc_result = processing.run("gdal:rastercalculator", raster_calc_params)
    raster_calc_layer = QgsRasterLayer(raster_calc_result['OUTPUT'], 'Raster Calculator Result')
    # Add the raster calculator result to the QGIS interface
    QgsProject.instance().addMapLayer(raster_calc_layer)
raster_calc()
