...
from PyQt5.QtWidgets import QDialog, QFileDialog, QTextEdit, QApplication, QWidget, QSizeGrip
...
from PyQt5.QtGui import QTextCursor, QTextCharFormat, QFont, QColor, QPainter, QBrush
import asyncio
...
class SpatialAnalysisAgentDockWidget(QtWidgets.QDockWidget, FORM_CLASS):
    ...
    # def check_and_install_libraries(self, filename):
    #     """Check and install required third-party Python libraries."""
    #     required_libraries = self.read_libraries_from_file(filename)
    #
    ...
    #                 subprocess.check_call(['python3', '-m', 'pip', 'install', library])
    #
...
class GPTRequestThread(QThread):
    ...
    def run(self):
        try:
            client = self.OpenAI(api_key=self.OpenAI_key)
