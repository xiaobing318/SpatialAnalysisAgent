...
def create_operation_prompt(task, data_path, selected_tool,selected_tool_ID, documentation_str):
    operation_requirement_str = '\n'.join([f"{idx + 1}. {line}" for idx, line in enumerate(constants.operation_requirement)])
    prompt =    f"Your role: {constants.operation_role} \n" + \
                f"Your mission: {constants.operation_task_prefix}: " + f"{task}\n\n" + "Using the following data paths:" + f"{data_path}" + \
...
def code_review_prompt(extracted_code, data_path, selected_tool_ID, documentation_str):
    operation_code_review_requirement_str = '\n'.join([f"{idx + 1}. {line}" for idx, line in enumerate(constants. operation_code_review_requirement)])
...
def execute_complete_program(code: str, try_cnt: int, task: str, model_name: str, documentation_str: str, data_path, review = True) -> (str, str):
    count = 0
    ...
    while count < try_cnt:
        ...
        try:
            ...
            if review:
                ...
                code = review_operation_code(extracted_code = code, data_path= data_path, documentation_str = documentation_str)
    return code, output_capture.getvalue()


def review_operation_code(extracted_code, data_path, documentation_str):
    OpenAI_key = load_OpenAI_key()
    ...
    print("\n\n")
    print(f"REVIEWED_CODE-------------------------------------------------------: \n\n")
    print("```python")
    reviewed_code = extract_code_from_str(review_str_LLM_reply_str, verbose=True)
    print(reviewed_code)
    print("```")
